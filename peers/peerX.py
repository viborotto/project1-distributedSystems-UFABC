...
peerXsocket = socket.socket()
# define the port on which you want connect
# nesse caso a porta que definimos no servidor
server_port = 1099
server_ip = '127.0.0.1'

peerXsocket.connect((server_ip, server_port))
address = peerXsocket.getsockname()
peer_ip, peer_port = peerXsocket.getsockname()
...
while True:
    ...
    if operacao == '1':
        ...
        peerXsocket.send(str(mensagem_join).encode())
        print("Enviada mensagem com OPERACAO JOIN: "+ str(mensagem_join))
        if peerXsocket.recv(1024).decode() == "JOIN_OK":
    ...
    ## SEARCH OPERATION: busca no servidor em quais peers o arquivo esta disponivel
        ...
        peerXsocket.send(str(mensagem_search).encode())
        peers_list = peerXsocket.recv(2048).decode()
        ...
        peerXsocket.send(str(mensagem_download).encode())

        peers_list_download = peerXsocket.recv(2048).decode()
        ...
        if peerXsocket.recv(2048).decode() == "yes":
            ## todo: fazer download
    ...
    # TODO: print("Arquivo "+ arquivo+ " baixado com sucesso na pasta /"+ diretorio)
        ...
        peerXsocket.send(operacao.encode())
        break
